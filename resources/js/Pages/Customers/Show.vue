                            ...
                            <div class="flex flex-wrap -m-2">
                                ...
                                <div class="p-2 w-full">
                                    <div class="relative">
                                        ...
                                        <div id="birthday" name="birthday" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" >
                                            <div v-if="customer.gender === 0">男性</div>
                                            <div v-else-if="customer.gender === 1">女性</div>
                                            <div v-else-if="customer.gender === 2">その他</div>
                                        </div>
                                ...
                                <div class="relative">
                                    ...
                                    <div id="memo" name="memo" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        {{ customer.memo }}
                                    </div>
                                </div>
                                </div>
                                <div class="p-10 ml-10 mt-2">
                                <Link as="button" :href="route('customers.edit', {customer: customer.id})" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-green-300 hover:text-black rounded text-lg">編集する</Link>
                                </div>
                                <div class="p-10 ml-10 mt-2">
                                    <button @click="deleteCustomer(customer.id)" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-red-600 hover:text-black rounded text-lg">削除する</button>
                                <!-- <Link as="button" :href="route('items.destory', {item: item.id})" class="flex mx-auto text-white bg-indigo-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">削除する</Link> -->
                                </div>
                            </div>
                            </div>
                        </div>
